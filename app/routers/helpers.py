...
from app.dependencies import get_admit_table, get_table
...
async def generate_uuids_reg(table: Table = Depends(get_table)):
...
async def generate_uuids_check_in(table: Table = Depends(get_admit_table)):
    for entry in table.all():
        if "UUID" not in entry["fields"].keys() or not entry["fields"]["UUID"]:
            generated_id = uuid.uuid4().hex
            table.update(entry["id"], {"UUID": generated_id})